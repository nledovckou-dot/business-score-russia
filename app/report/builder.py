...
_PLACEHOLDER_REASONS: dict[str, str] = {
    "M1": "Отраслевые данные не найдены в открытых источниках. Рекомендуется проверить РБК Исследования или Infoline.",
    "M2": "Данные о регуляторике не были собраны для данного типа бизнеса.",
    "M3": "Недостаточно данных о технологических трендах отрасли.",
    "M4": "Данные о рынке труда: HH API не вернул результатов для указанных позиций.",
    "C1": "Для построения перцептуальной карты нужны данные хотя бы 3 конкурентов.",
    "C2": "Профили конкурентов не удалось обогатить данными из ФНС/Rusprofile.",
    "C3": "Недостаточно данных для радара: менее 3 конкурентов с оценками.",
    "C7": "Данные о жизненном цикле не определены — нужна финансовая динамика за 3+ года.",
    "C8": "Каналы продаж не найдены: проверено менее 3 источников.",
    "P2": "Финансовые данные не найдены в ФНС. Для ПАО/крупных компаний нужен платный API (СБИС).",
    "P3": "SWOT-анализ не сгенерирован: недостаточно входных данных о компании.",
    "P4": "Digital-аудит: соцсети компании не найдены через web search. Требуется ручная проверка.",
    "P7": "Отзывы: скрапинг 2ГИС/Яндекс.Карт не настроен для данного региона.",
    "P8": "Тендеры: zakupki.gov.ru не вернул данных по ИНН компании.",
    "P10": "Доля рынка: нет достоверных данных для расчёта (TAM не определён из отраслевых источников).",
    "B1": "Совет директоров: рецензия не выполнена (API кредиты исчерпаны или модель недоступна).",
    "O1": "Фаундеры: данные ЕГРЮЛ не найдены или ИНН не определён.",
    "O2": "Мнения экспертов: web search не нашёл цитат лидеров отрасли с верифицированными источниками.",
}
...
def _render_placeholder(block_id: str) -> str:
    ...
    reason = _PLACEHOLDER_REASONS.get(block_id, "Автоматический сбор данных не вернул результатов для этой секции.")
    return (
        '<div class="block-placeholder">'
        '<div class="ph-icon">&#128269;</div>'
        f'<div class="ph-title">{name}</div>'
        f'<div class="ph-text">{reason}</div>'
        '</div>'
    )
